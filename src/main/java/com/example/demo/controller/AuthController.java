...
public class AuthController {
    ...
    @PostMapping("/login")
    public ResponseEntity<?> login(final @Valid @RequestBody LoginRequest req) {
        try {
            ...
            val access = jwtService.generateAccessToken(username, roles);
            val saved = refreshTokenService.createOrReplace(username, roles);
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                Map.of("error", "Неверные учетные данные")
            );
        }
    }
    ...
    public ResponseEntity<?> refresh(
        ...
        if (StringUtils.isBlank(refresh)) {
            return ResponseEntity.badRequest().body(
                Map.of("error", "refreshToken обязателен")
            );
        }
        ...
        if (stored.isEmpty()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                Map.of("error", "Неверный или просроченный refresh token")
            );
        }
        ...
        if (!stored.get().getUsername().equals(username)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                Map.of("error", "Несовпадение refresh token")
            );
        }

        val roles = jwtService.extractRoles(refresh);
        val newRefresh = refreshTokenService
            .rotate(username, refresh, roles)
            .getToken();
    ...
    public ResponseEntity<?> logout(
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            if (
                ...
                !jwtService.isRefreshToken(accessToken)
            ) {
        ...
        if (
            ...
            jwtService.isRefreshToken(refresh)
        ) {
            if (
                username != null &&
                !jwtService.extractUsername(refresh).equals(username)
            ) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                    Map.of("error", "Несовпадение refresh token")
                );
